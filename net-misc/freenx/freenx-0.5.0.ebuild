...
src_unpack() {
	...
	# Change to the new path to allow both FreeNX and NoMachine servers.
	sed -i 's/NX_DIR=\/usr\/NX/NX_DIR=\/usr\/NX\/freenx/' nxloadconfig
...
src_install() {
	...
	for x in ${ROOT}usr/NX/bin/*; do
		if [ ${x##*\/} != "nxclient" ] && [ ${x##*\/} != "nxnode" ] && \
		   [ ${x##*\/} != "nxprint" ] && [ ${x##*\/} != "nxserver" ] ; then
			dosym ${x} ${NX_DIR}/bin/.
		fi
	done

	dosym ${ROOT}usr/NX/lib ${NX_DIR}/lib
...
pkg_postinst () {
	...
	ewarn "NOTE: Please read the docs on how to have both !M and FreeNX severs running at the same time."
}
